  ...
  (event) => (dispatch, getState) => {
    ...
    switch (reason) {
      case 'connection error':
        if (!willReconnect) {
          dispatch(showError('Error while connecting to Dhaksha server'));
        }
      ...
      case 'connection timeout':
        if (!willReconnect) {
          dispatch(showError('Timeout while connecting to Dhaksha server'));
        }

        break;

      case 'io client disconnect':
        dispatch(showNotification('Disconnected from Dhaksha server'));
        break;
      ...
      case 'transport close':
        dispatch(showError('Dhaksha server closed connection unexpectedly'));
        break;

      case 'ping timeout':
        dispatch(showError('Connection to Dhaksha server lost'));
        break;
  ...
  (dispatch) => ({
    ...
    onLocalServerError(message, wasRunning) {
      const baseMessage = wasRunning
        ? 'Dhaksha server died unexpectedly'
        : 'Failed to launch local Dhaksha server';
      dispatch(setCurrentServerConnectionState(ConnectionState.DISCONNECTED));
